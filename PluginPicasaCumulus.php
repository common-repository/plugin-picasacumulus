<?php
/*
Plugin Name: PluginPicasaCumulus
Plugin URI: http://www.dimgoto.com/open-source/wordpress/plugins/plugin-picasacumulus
Description: Display images cumulus from feed Picasa. Use FlickrWidget powered by Roy Tanck. Album public, private, protected
Version: 1.1.0
Author: Dimitri GOY
...
/**
 * Classe PluginPicasaCumulus.
 *
 * Display Picasa Photo Cumulus.
 *
 * @package Plugins
 * @subpackage PicasaCumulus
 * @version 1.1.0
 * @author Dimitri GOY
...
class PluginPicasaCumulus {
	...
	public function init() {
		...
		if (is_admin()) {
			...
			add_action('wp_ajax_picasacumulus_authenticate', array($this, 'ajax_authenticate'));
			add_action('wp_ajax_picasacumulus_list', array($this, 'ajax_list'));
			add_action('wp_ajax_picasacumulus_configure', array($this, 'ajax_configure'));
			...
			&& $_GET['page'] == $this->_pluginname) {
				...
				wp_enqueue_script(strtolower($this->_pluginname) . '-admin', $this->_jsdir . '/pluginpicasacumulus.js', array(), false);
	...
	public function menu() {
		...
		add_submenu_page('plugins.php',
				__('Picasa Cumulus', $this->__pluginname),
				__('Picasa Cumulus', $this->__pluginname),
				'activate_plugins',
				$this->_pluginname,
				array($this, 'control'));
	...
	public function control() {
		...
		$html .= '<h2>' . __('Configuration PicasaCumulus', $this->pluginname) . '</h2>';
		$html .= '<br/>';
		...
		$html .= '</tr>';
		$html .= '<tr>';
		$html .= '<th scope="row"><label for="' . strtolower($this->_pluginname) . '-configure-bgcolor">' . __('Couleur', $this->_pluginname) . '<span class="required">*</span></label></th>';
		$html .= '<td><input type="text" id="' . strtolower($this->_pluginname) . '-configure-bgcolor" name="' . strtolower($this->_pluginname) . '-configure-bgcolor"/></td>';
		$html .= '</tr>';
		$html .= '</tbody>';
		$html .= '</table>';
		$html .= '</form>';
		...
		if (isset($id)) {
			...
			&& array_key_exists('albums', $options)) {
				...
				if (!is_null($thepicasa)) {
					...
					if (!empty($thepicasa['title'])) {
						$html .= '<h2>' . $thepicasa['title'] . '</h2>';
						$html .= '<br/>';
					}

					$html .= '<object type="application/x-shockwave-flash" data="http://media.roytanck.com/flickrwidget.swf"" width="' . $thepicasa['width'] . '" height="'. $thepicasa['height'] . '">';
					$html .= '<param name="movie" value="http://media.roytanck.com/flickrwidget.swf" />';
					$html .= '<param name="bgcolor" value="' . $thepicasa['bgcolor'] . '" />';
					$html .= '<param name="AllowScriptAccess" value="always" />';
					$html .= '<param name="flashvars" value="feed=http://picasaweb.google.com/data/feed/base/user/' . $thepicasa['user'] . '/albumid/' . $thepicasa['id_album'];
					if (!is_null($authey)) {
						$html .= '%26authkey%3D' . $thepicasa['authkey'];
					}
					$html .= '?alt=rss&kind=photo&hl=fr';
					$html .= '<p>';
					$html .= '<a href="http://www.roytanck.com">Roy Tanck</a>';
					$html .= 'Flick Widget requires Flash Player 9 or better.';
					$html .= '</p>';
					$html .= '</object>';
	...
	public function ajax_authenticate() {
		...
		|| empty($_POST['password'])) {
			...
			header("Status: 400 Bad Request", true, 400);
			die(__('Extension cUrl non active!', $this->_pluginname));
		} else {
			try {
				$params = array(
	...
	public function ajax_configure() {
		...
		|| empty($_POST['id_album'])) {
			...
			$bgcolor = (!isset($_POST['bgcolor']) || empty($_POST['bgcolor'])) ? '#ffffff' : $_POST['bgcolor'];
			$options = get_option(strtolower($this->_pluginname));
